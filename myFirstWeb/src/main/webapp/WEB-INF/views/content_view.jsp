		...
		<div id="contents">
			...
			<table>
				<c:forEach items="${content_view}" var="dto">
					...
					<tr>
						<td>글쓴이 :${dto.writer} 날짜 :${dto.date } 조회 :${dto.hit }</td>
					</tr>
					<tr>
						<td>${dto.content }</td>
					</tr>
			...
			<form name="reply" action="reply" method="post">
				<table>
					...
					<c:forEach items="${content_view}" var="content_viewdto">
						...
						<td><input type="hidden" name="num" value="${content_viewdto.num }"/></td>
			...
			<form name="replyandreply" action="replyandreply" method="post">
				<table>
					<c:forEach items="${content_replyview}" var="replydto">
						<tr>
							<td>글쓴이 사진</td>
							<td><p>${replydto.datetime}</p>
								<p>${replydto.replycontent }</p></td>
						</tr>
						<tr>
							<td>추천${replydto.agreement } 반대 ${replydto.opposition } 답글쓰기</td>
						</tr>
					</c:forEach>
